...
// Bypass Entry Point
extern "C" void* OneCoreCoreInit(void* arg);
...
static void install_all_hooks(JavaVM* vm) {
    ...
    pthread_create(&bypass_thread, nullptr, OneCoreCoreInit, nullptr);
    pthread_detach(bypass_thread);
    ...
    if (vm && vm->GetEnv((void**)&env, JNI_VERSION_1_6) == JNI_OK) {
         ...
         // Initialize JNI Hook Engine with API level
         #include <sys/system_properties.h>
         char sdk_ver[PROP_VALUE_MAX];
         __system_property_get("ro.build.version.sdk", sdk_ver);
         int api_level = atoi(sdk_ver);
         OneCore::JniHook::InitJniHook(env, api_level);

         OneCore::installJniHooks(env);
         
         // Initialize functional JNI hooks
         OneCore::BinderHook::init(env);
         OneCore::DexFileHook::init(env);
         OneCore::RuntimeHook::init(env);
         OneCore::UnixFileSystemHook::init(env);
         OneCore::VMClassLoaderHook::init(env);
    }
...
JNI_OnLoad(JavaVM* vm, void* reserved) {
    ...
    if (!registerNativeMethods(env, "com/onecore/sdk/core/OneCoreNativeCore", gMethods, sizeof(gMethods) / sizeof(gMethods[0]))) {
        LOGE("Failed to register OneCoreNativeCore methods");
    }
...
extern "C" JNIEXPORT void JNICALL
Java_com_onecore_sdk_core_OneCoreNativeCore_hideXposed(JNIEnv *env, jclass clazz) {
    LOGD("set hideXposed");
    OneCore::VMClassLoaderHook::hideXposed();
}

extern "C" JNIEXPORT void JNICALL
Java_com_onecore_sdk_core_OneCoreNativeCore_init(JNIEnv *env, jclass clazz, jint api_level) {
    LOGD("NativeCore init: API %d", api_level);
    JavaVM* vm;
    env->GetJavaVM(&vm);
    setBoxCoreEnv(vm, env, api_level);
    JniHook::InitJniHook(env, api_level);
}

extern "C" JNIEXPORT void JNICALL
Java_com_onecore_sdk_core_OneCoreNativeCore_addIORule(JNIEnv *env, jclass clazz, jstring target_path, jstring relocate_path) {
    LOGD("addIORule");
...
extern "C" JNIEXPORT void JNICALL
Java_com_onecore_sdk_core_OneCoreNativeCore_enableIO(JNIEnv *env, jclass clazz) {
    LOGD("enableIO");
    IO::init(env);
    nativeHook(env);
}
...
static JNINativeMethod gMethods[] = {
    {"hideXposed", "()V",      (void *) Java_com_onecore_sdk_core_OneCoreNativeCore_hideXposed},
    {"addIORule",  "(Ljava/lang/String;Ljava/lang/String;)V", (void *) Java_com_onecore_sdk_core_OneCoreNativeCore_addIORule},
    {"enableIO",   "()V",(void *) Java_com_onecore_sdk_core_OneCoreNativeCore_enableIO},
    {"init",       "(I)V",(void *) Java_com_onecore_sdk_core_OneCoreNativeCore_init},
};
...
static int registerNativeMethods(JNIEnv *env, const char *className, JNINativeMethod *methods, int numMethods) {
    ...
    if (env->RegisterNatives(clazz, methods, numMethods) < 0) return JNI_FALSE;
    return JNI_TRUE;
}
